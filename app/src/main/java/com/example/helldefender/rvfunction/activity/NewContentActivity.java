...
public class NewContentActivity extends BaseActivity implements View.OnClickListener {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        newsContentListener = new SubscriberOnNextListener<NewsContentBean>() {
            ...
            public void onNext(NewsContentBean newsContentBean) {
                mWebView.loadDataWithBaseURL("file:///android_asset/", ActivityUtil.structHtml(newsContentBean), "text/html", "UTF-8", null);
            }
        };
    ...
    private void httpRequestGet() {
        //获取不到数据的原因:单例模式
        HttpMethods.getInstance().getNewsContent(new ProgressSubscriber<NewsBean>(newsContentListener, this), newsId);
        HttpMethods.getInstance().getNewsExtra(new ProgressSubscriber<NewsExtraBean>(newsExtraListener,this) ,newsId);
    }
