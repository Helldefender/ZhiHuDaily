...
import com.example.helldefender.rvfunction.ui.adapter.CommentsRvAdapter;
...
public class CommentsFragment extends BaseFragment {
            ...
            public void onItemClickListener(View view, int position) {
                if (longData.size() == 0) {
                    if (position == longData.size() + 3) {
                        HttpManager.getInstance().getNewsShortComments(new ProgressSubscriber<NewsCommentsBean>(shortCommentsListener, getHoldingActivity()), newsId);
                        return;
                    }
                }
    ...
    private void httpRequest() {
        HttpManager.getInstance().getNewsLongComments(new ProgressSubscriber<NewsCommentsBean>(longCommentsListener, getHoldingActivity()), newsId);
    }
}
